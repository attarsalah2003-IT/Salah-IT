/**
 * This code was generated by Builder.io.
 */
import React from "react";
import { Button } from "./Button";
import { FeatureList } from "./FeatureList";

interface WhatWeAreDoingProps {
  title: string;
  description: string;
  features: string[];
}

export const WhatWeAreDoing: React.FC<WhatWeAreDoingProps> = ({
  title,
  description,
  features,
}) => {
  return (
    <section className="pr-px pl-20 mt-32 w-full bg-neutral-900 max-md:pl-5 max-md:mt-10 max-md:max-w-full">
      <div className="flex gap-5 max-md:flex-col">
        <div className="flex flex-col w-[31%] max-md:ml-0 max-md:w-full">
          <div className="flex flex-col items-start self-stretch my-auto w-full font-bold max-md:mt-10">
            <h2 className="text-5xl leading-10 text-stone-50">
              {title.split(" ").map((word, index) => (
                <React.Fragment key={index}>
                  {word}
                  <br />
                </React.Fragment>
              ))}
            </h2>
            <div className="flex flex-col items-start self-stretch pl-2 mt-8 text-sm leading-6 text-neutral-400">
              <div className="shrink-0 border-blue-400 border-solid border-[3px] h-[3px] w-[132px]" />
              <p className="self-stretch mt-5">{description}</p>
              <FeatureList features={features} />
            </div>
            <div className="flex gap-4 mt-11 w-full text-base text-white max-md:mt-10">
              <Button variant="primary">See More</Button>
              <Button variant="secondary">Contact Us</Button>
            </div>
          </div>
        </div>
        <div className="flex flex-col ml-5 w-[69%] max-md:ml-0 max-md:w-full">
          <div className="flex grow text-xs leading-none whitespace-nowrap text-neutral-400 max-md:mt-10 max-md:max-w-full">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/7b7177ee7be56471d98d61c3f7daed83b05924e2464a158d3d5e1a9b11ac208d?placeholderIfAbsent=true&apiKey=a9e86b264a2b427cb02fa9112891bf11"
              alt=""
              className="object-contain grow shrink-0 aspect-[1.14] basis-0 w-fit max-md:max-w-full"
            />
            <span className="self-start mt-[482px] max-md:mt-10">Expert</span>
          </div>
        </div>
      </div>
    </section>
  );
};
